...
/* ------------------ Utils: Poissons Japon depuis l’Excel ------------------ */
async function loadGroupedJaponSpecies() {
...
export default function Japon() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await loadGroupedJaponSpecies();
        if (!mounted) return;
...
/* ------------------ Visibilité: vague desktop (palette Japon) ------------------ */
