...
/* ------------------ Utils: Poissons Thaïlande depuis l’Excel ------------------ */
async function loadGroupedThailandeSpecies() {
  ...
  // tolère "thailande" / "thaïlande" / "thailand"
  const targets = new Set(["thailande", "thaïlande", "thailand"]);
  const th = rows.filter((r) => targets.has(String(r.pays || "").toLowerCase().trim()));
...
export default function Thailande() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await loadGroupedThailandeSpecies();
        if (!mounted) return;
  ...
  return (
    <div className="w-full">
      ...
      {/* Carte + panneau de droite (⚠️ PAS de Reveal ici) */}
      <div
      ...
      >
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl max-w-[1200px] mx-auto p-4 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-3 rounded-xl overflow-hidden">
            <CarteAvecDonnees country="thailande" regionFilter={regionFilter} mapId="map-thailande" />
          </div>
                  ...
                  <label className="inline-flex items-center gap-2 text-gray-800">
                    <input
                      ...
                      onChange={() => {
                        ...
                        const el = document.getElementById("map-thailande");
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <Reveal zoomOnVisible className="ml-[6%]">
              <p className="mb-8 text-sm md:text-base text-gray-700 max-w-4xl text-justify">
                ...
                <span className="text-[#1113a2] font-bold">réduire l’impact</span>{" "}
                de la plongée sur l’environnement marin (Similan, Koh Tao, Koh Lanta…),
                en formant les plongeurs et en appliquant des protocoles responsables.
              </p>
            </Reveal>
              ...
              <Reveal delay={120} zoomOnVisible>
                <div className="group rounded-xl shadow-md border border-gray-200 overflow-hidden transition-transform duration-300 hover:scale-[1.02] hover:shadow-lg">
                  ...
                  <div className="p-4 text-sm text-gray-700 transition-transform duration-300 group-hover:scale-[1.01]">
                    ...
                    Pratiques qui{" "}
                    <span className="text-[#1113a2]">limitent les dégâts</span>{" "}
                    sur coraux et faune.
                  </div>
                </div>
              </Reveal>
        ...
        <div className="bg-gray-300 py-12 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="ml-[4%] mb-4 text-sm md:text-base text-gray-700 max-w-3xl">
              La côte Andaman (ouest) est souvent meilleure ~novembre–avril, le Golfe de Thaïlande
              (est) peut être plus favorable en été.
            </p>
      ...
      <Reveal>
        <div className="bg-gray-100 py-12 px-6">
          ...
          <p className="ml-[10%] mb-6 text-sm md:text-base text-gray-700 max-w-4xl text-justify">
            ...
            <span className="text-[#1113a2] font-semibold">emblématiques ou rares</span>{" "}
            que l’on rencontre en Thaïlande.
          </p>
...
/* ------------------ Visibilité: vague desktop (palette Thaïlande) ------------------ */
function MonthsWaveThailand() {
  const isTouch = useIsTouch(); // pas de vague sur mobile/tablette
  const months = ["Jan","Fév","Mar","Avr","Mai","Juin","Juil","Août","Sept","Oct","Nov","Déc"];
  // Palette indicative Thaïlande (Andaman souvent top nov–avr ; été plus variable sauf Golfe)
  const colors = [
    "bg-green-500","bg-green-500","bg-green-500","bg-green-500",
    "bg-yellow-400","bg-orange-400","bg-orange-400","bg-orange-400",
    "bg-orange-400","bg-yellow-400","bg-green-500","bg-green-500"
  ];
