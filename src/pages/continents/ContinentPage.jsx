...
/* Pays : quelques corrections FR/slug (fallback = libellé source) */
const COUNTRY_FR = {
  "curacao": ["Curaçao","curacao"], "curaçao": ["Curaçao","curacao"],
...
const CENTROIDS = {
  "egypt":[30.8,26.8],"morocco":[-7.1,31.8],"south-africa":[22.9,-30.6],
  "tanzania":[34.9,-6.37],"mozambique":[35.5,-18.66],"mauritius":[57.55,-20.35],
  "cabo-verde":[-23.6,15.1],"seychelles":[55.49,-4.68],

  "france":[2.2,46.2],"spain":[-3.7,40.4],"italy":[12.6,41.9],"portugal":[-8.2,39.4],
  "greece":[23.7,38],"switzerland":[8.2,46.8],"sweden":[18.6,60.1],
  "norway":[8.47,60.47],"netherlands":[5.3,52.1],"belgium":[4.5,50.6],
  "croatia":[16.4,45.1],"slovenia":[14.5,46.1],"montenegro":[19.3,42.7],
  "romania":[25,45.9],"poland":[19.1,52.2],"bulgaria":[25.5,42.7],
  "united-kingdom":[-1.5,52.3],"ireland":[-8.2,53.4],"iceland":[-19,64.9],
  "cyprus":[33.4,35.1],"malta":[14.4,35.9],"reunion":[55.53,-21.12],

  "united-states":[-98.6,39.8],"canada":[-106.3,56.1],"mexico":[-102.5,23.6],
  "dominican-republic":[-70.16,18.73],"bahamas":[-76,24.25],"belize":[-88.5,17.2],
  "aruba":[-69.97,12.52],"curacao":[-68.99,12.17],"bonaire":[-68.26,12.2],
  "barbados":[-59.56,13.19],"panama":[-80.78,8.54],"honduras":[-86.24,15.2],
  "saint-lucia":[-60.98,13.91],"grenada":[-61.68,12.12],
  "turks-and-caicos-islands":[-71.8,21.69],"martinique":[-61,14.6],
  "u-s-virgin-islands":[-64.8,18.35],"french-polynesia":[-149.4,-17.6],

  "indonesia":[113.9,-0.79],"malaysia":[109.1,3.14],"philippines":[121.77,12.88],
  "thailand":[101,15.87],"vietnam":[108.3,14.06],"japan":[138.25,36.2],
  "taiwan":[120.96,23.7],"south-korea":[127.77,35.91],"india":[78.96,20.59],
  "jordan":[36.24,30.59],"saudi-arabia":[45.08,23.89],"united-arab-emirates":[54.38,24.45],
  "israel":[34.85,31.05],"maldives":[73.5,3.2],"timor-leste":[125.73,-8.87],

  "australia":[133.78,-25.27],"new-zealand":[174.89,-40.9],
  "cook-islands":[-159.78,-21.24],"fiji":[178.07,-17.71],"palau":[134.58,7.52],
...
export default function ContinentPage() {
  ...
  const countries = useMemo(() => {
    ...
    const kCont = ["continent","Continent","CONTINENT","continent_en","Continent_en"].find(k => keys.includes(k));
    ...
    // fusion FR/EN : on passe chaque ligne par l'alias
    const sameCont = rows.filter(r => {
      const cont = CONTINENT_ALIAS[toSlug(String(r[kCont] || ""))];
      return cont === canon;
    });

    const acc = new Map(); // key pays normalisé
    sameCont.forEach(r => {
      ...
      if (lat !== null && lon !== null) { o.lats.push(lat); o.lons.push(lon); }
    });

    return Array.from(acc.entries()).map(([pSlug, o]) => {
      ...
      if (o.lats.length) {
        ...
        const [lo, la] = CENTROIDS[pSlug]; lat = la; lon = lo;
      }
  ...
  }, [slug]);   // keep slug (or canon) in deps
  ...
  useEffect(() => {
    ...
    const fit = () => {
      if (canon === "oceania") {
        // One clean Pacific view (no duplicated worlds, no cut islands)
        map.easeTo({
          center: [165, -20],   // ~between Australia / Fiji / NZ
          zoom: 2.5,            // tweak: 3.0–3.6 depending on your UI
          duration: 700,
        });
        return;
      }
  ...
  return (
    <div className="w-full">
      <section className="py-12 text-center bg-gray-100">
        <h1 className="text-3xl md:text-4xl font-extrabold text-[#1113a2]">{titleFR}</h1>
      </section>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {filtered.map(c => (
              <button
                ...
                title={`${c.label} • ${c.count} centre(s)`}
              >
            ...
            {!filtered.length && <div className="text-sm text-gray-500">Aucun pays trouvé.</div>}
          </div>
        </div>
      </section>
    </div>
