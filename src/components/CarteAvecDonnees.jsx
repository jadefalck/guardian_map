...
const hasValidWebsite = (w) =>
  !!w && typeof w === "string" &&
  !/^pas\s*de\s*lien$/i.test(w.trim()) && w !== "#" &&
...
export default function CarteAvecDonnees({ country, regionFilter, mapId }) {
  ...
  const makeMarkers = useCallback((rows) => {
      ...
      const popupHTML = `
        <div style="font-family:system-ui,Inter,Roboto; font-size:13px; max-width:280px; line-height:1.5;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            ...
            <span style="display:inline-block;padding:4px 8px;border-radius:9999px;background:${bg};color:${color};font-size:12px;font-weight:800;">
              ${label}
            </span>
          </div>
    ...
    if (rows.length > 0) {
      ...
      rows.forEach((d) => b.extend([Number(d.longitude), Number(d.latitude)]));
      map.fitBounds(b, { padding: 60, maxZoom: 9, duration: 400, essential: true });
    }
  }, []);

  // create the map ONCE per country
  useEffect(() => {
    ...
    map.on("load", () => {
      const base = getDataByCountry(country).filter(
    ...
    return () => {
      try { markersRef.current.forEach((m) => m.remove()); map.remove(); } catch {}
      markersRef.current = [];
      mapRef.current = null;
    };
  }, [country, makeMarkers]);

  // update markers + move camera when regionFilter changes (NO map reload)
  useEffect(() => {
    ...
    makeMarkers(rows);
  }, [regionFilter, country, makeMarkers]);

  return (
    <div
      id={mapId || undefined}
      ref={containerRef}
      style={{ height:"600px", width:"100%", borderRadius:"1rem", overflow:"hidden" }}
    />
  );
}
